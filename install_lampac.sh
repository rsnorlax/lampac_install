...
# Проверка, что пользователь имеет права sudo
if ! sudo -n true 2>/dev/null; then
    echo "Пользователь $USER не имеет прав sudo или требуется ввод пароля."
    echo "Добавьте пользователя в группу sudo: sudo usermod -aG sudo $USER"
    exit 1
fi
